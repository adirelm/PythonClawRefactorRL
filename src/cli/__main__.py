...
def _cmd_cost() -> int:
    ...
    print(f"Skills corpus token volume (cl100k_base): {vol['total']} tokens")
    for layer, count in vol["by_layer"].items():
        print(f"  {layer}: {count}")
    print(f"Lazy-load saving (L1-only vs all): {vol['lazy_load_saving']:.1f}x")
    return 0
