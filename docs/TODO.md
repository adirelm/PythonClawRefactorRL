...
| 4 | §2.4 Cost + ablations + essay (tiktoken cost panel, α/β/γ/P_skills ablation matrix ≥5 seeds, ΔReward summary, 2500–3000 word essay) | ✅ | `docs/COST_ANALYSIS.md` (§0 Skills-token + PPO-runtime, D8), `results/ablations/` (81 cells × 5 seeds, 405/405 ok), `docs/ANALYSIS.md` (D7 ΔReward), `docs/ESSAY.md` (≈3,000 words), `notebooks/analysis.ipynb`, commit `phase4(rc4)` |
...
|----|---------|--------|------------------|
| T1.1 | Vendor PythonClaw shim (+ adapter Protocol per ADR-001) | ✅ | `src/pythonclaw_shim/skill.py`, `src/pythonclaw_shim/registry.py` (registry already implements the ADR-001 adapter Protocol surface in-tree; no separate `src/adapters/` facade required), commit `0165fa2` |
| T1.2 | Run GRAPHIFY on Skills/ (local_impl real, not stub) | ✅ | `src/graphify/local_impl.py`, `results/graphify_output.gpickle` (gpickle is the canonical graph artefact; JSON export deferred to Phase 4 because notebook + ablation consumers read the gpickle directly via the SDK), commit `0165fa2` |
| T1.3 | Build Obsidian Vault (vault writer from GRAPHIFY output) | ✅ | `scripts/build_vault.py`, `src/services/vault_writer.py`, `results/vault/` (vault writer lives under `src/services/` not a separate `src/obsidian/` module — the writer is a graph-consuming service, not its own bounded context), commit `0165fa2` |
| T1.4 | Capture obsidian_before.png (pre-refactor hero shot) | ✅ | `scripts/capture_obsidian_stub.py`, `results/figures/obsidian_before.png` (kept under `results/figures/` alongside other PNG deliverables; no separate `results/obsidian/` dir — single figures dir is the canonical location), commit `0165fa2` |
| T1.5 | Author `docs/SKILLS_ARCHITECTURE.md` (F15) | ✅ | `docs/SKILLS_ARCHITECTURE.md`, commit `0165fa2` |
| T1.6 | Run lazy-load invariant tests (ADR-005 semantics) | ✅ | `tests/architecture/test_lazy_load_invariants.py`, commit `0165fa2` |
...
| T01-03 | Implement `src/graphify/adapter.py` per ADR-002 (Protocol surface) + `src/graphify/local_impl.py` (concrete `as_networkx()` / `as_pyvis_html()` impl) — note: original draft cited `src/adapters/graphify_adapter.py` but ADR-002 was finalised to live under `src/graphify/` alongside the parser/builder it wraps (cohesion-of-bounded-context); no separate `src/adapters/` package | Implementing GraphifyAdapter facade | 1 | +90 | §2.1-adapter,ADR-002 | `test_graphify_adapter_methods_present` + `test_adapter_returns_networkx_digraph` |
...
| T2.2 | Action space (split_module, merge_modules, rewire_edge, noop; A_max=45057) per `docs/ACTION_DESIGN.md` | ✅ | `src/env/actions.py`, commit `ec1288a` |
| T2.3 | Action masking (pre-softmax logit → −∞, Huang & Ontañón 2022) | ✅ | `src/env/action_mask.py`, commit `ec1288a` |
| T2.4 | Canonical reward `R_t = α·ΔModularity + β·ΔCohesion − γ·Coupling_Penalty + P_skills` (α=1.0, β=1.0, γ=0.5, P_skills=−5.0) | ✅ | `src/env/reward.py`, commit `ec1288a` |
| T2.5 | Custom graph env (`step` / `reset`) — **NO Gymnasium import** (brief §2.2 ban) | ✅ | `src/env/skills_graph_env.py`, commit `ec1288a` |
| T2.6 | Metrics services — modularity, cohesion, coupling | ✅ | `src/services/metrics/modularity.py`, `src/services/metrics/cohesion.py`, `src/services/metrics/coupling.py`, commit `1c317ef` |
| T2.7 | Centrality service + CentralityScheduler (degree per step; betweenness exactly 2×/seed per ADR-006) | ✅ | `src/services/centrality.py`, commit `1c317ef` |
| T2.8 | Architecture test `tests/architecture/test_env_no_gym.py` **active** (no `xfail`) — AST walk of `src/env/` rejecting `import gymnasium` | ✅ | `tests/architecture/test_env_no_gym.py`, commit `ec1288a` |
| T2.9 | Architecture test `tests/architecture/test_betweenness_call_count.py` **active** — patches `networkx.betweenness_centrality`, asserts 2 calls/seed | ✅ | `tests/architecture/test_betweenness_call_count.py`, commit `1c317ef` |
| T2.10 | Episode smoke script — random-policy rollout from `reset()` to `done=True` | ✅ | `scripts/run_episode_smoke.py`, commit `ec1288a` |
...
## §3.3 Phase 3 — §2.3 PPO + GAE (✅ workflow-level T3.1–T3.9 landed)
...
### §3.3.0 Phase 3 workflow-level tasks (T3.1–T3.9 ✅ landed)

The Phase 3 build workflow tracks these nine top-level deliverables, all
...
| T03-07 | Implement `src/sdk/sdk.py` — single facade exposing `build_graph()`, `train_ppo()`, `evaluate()`, `get_metrics()`, `save_run()` (SDK ships as a package `src/sdk/` not a flat module; `build_skills_graph` already wired in Phase 1, train/evaluate stubs remain pending) | Implementing SDK facade | 3 | +140 | §2.3-sdk | `test_sdk_is_single_entry_point` (CLI, notebook import only SDK) |
