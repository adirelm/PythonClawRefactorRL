...
| §2.2 — **F10** betweenness centrality | Centrality service: Degree per MDP step (cheap); Betweenness **exactly twice per seed** (training start + training end) via `CentralityScheduler`, aggregated across ≥5 seeds {42, 7, 123, 314, 271} with mean ± std + 95% CI for both endpoints AND Δ. Phase-3-rework R2 5-seed retrain landed R1 NOOP-slot fix (`5dd14ca`) but the retrain run itself only completed seed_42 before the run-window stop hook fired — seed_7 was mid-rollout, seeds 123 / 314 / 271 never started. On-disk evidence: only `results/training/seed_42/` exists; no `aggregate.json`; verdicts dir empty. R1 fix is believed correct (defensive NOOP fallback in `PolicyNet.get_action`) but is UNVALIDATED on seeds 123/314/271 because they did not run. See **Known gaps** in `docs/TODO.md`. | `src/services/centrality.py` | `tests/architecture/test_betweenness_call_count.py`, `tests/architecture/test_seeds_discipline.py` | `docs/adr/ADR-006-multi-seed-eval-discipline.md`, `results/figures/betweenness_ci.png`, `results/data/betweenness_table.csv` | `71f0213` | 🟡 in-progress (1 of 5 seeds on disk post-R2 retrain; R1 NOOP fix `5dd14ca` landed but unvalidated on hang-prone seeds) |
