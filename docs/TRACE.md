...
| §2.2 — **F10** betweenness centrality | Centrality service: Degree per MDP step (cheap); Betweenness **exactly twice per seed** (training start + training end) via `CentralityScheduler`, attempted across all 5 sealed seeds {42, 7, 123, 314, 271} with mean ± std + 95% CI for both endpoints AND Δ. Post-rework outcome (R1 NOOP fix `5dd14ca` + isolated-subprocess wrapper `scripts/train_5seed_isolated.py`): **3 of 5 seeds train cleanly** (42, 7, 271 each in ~13–14s at 256 steps); **2 of 5 seeds (123, 314) hit a SECOND undiagnosed slow path** in PPO iter 2+ that emerges even with R1 in place and even when each seed runs in its own subprocess — killed by the 120s per-seed wall-clock budget. `aggregate.json` carries `attempted_seeds=[42,7,123,314,271]` so the n_seeds gap is auditable; chart + CSV reflect honest n=3 with dof=2 CI (column `n_seeds`). Phase-4 root-causes the residual hang. See **Known gaps** in `docs/TODO.md`. | `src/services/centrality.py`, `scripts/train_5seed_isolated.py` | `tests/architecture/test_betweenness_call_count.py`, `tests/architecture/test_seeds_discipline.py` | `docs/adr/ADR-006-multi-seed-eval-discipline.md`, `results/figures/betweenness_ci.png` (n=3), `results/data/betweenness_table.csv` (n=3) | `71f0213` | 🟡 in-progress (3 of 5 seeds OK; 123/314 hang on second slow path — Phase-4 RC&FIX) |
| §2.3 | PPO (Schulman 2017, arXiv:1707.06347) + GAE(λ) (Schulman 2016, arXiv:1506.02438) with proper advantage normalisation; SB3 buffer spike timeboxed 2h, padding-to-V_max=512 fallback gated by ADR-008. | `src/services/ppo_trainer.py`, `src/services/gae_buffer.py`, `src/model/policy_net.py`, `src/model/encoder.py` | `tests/unit/test_gae_math.py`, `tests/integration/test_ppo_one_update.py` | `results/training/ppo_learning_curve.png`, `docs/prd/PRD-PPO.md`, `docs/prd/PRD-GAE.md`, `docs/adr/ADR-008-sb3-variable-v-buffer.md` | `71f0213` | ✅ trainer + GAE + policy + encoder landed Phase 3 |
| §2.3 — **F11** PPO trainer | PPO clipped-surrogate trainer with ε=0.2 (FIXED), value loss, entropy bonus. Wraps our 4-tuple `SkillsGraphEnv` directly — **does NOT depend on gymnasium** (brief §2.2 ban extends to trainer). Step contract: `state, reward, done, info = env.step(action)`. | `src/services/ppo_trainer.py` | `tests/unit/services/test_ppo_trainer.py`, `tests/integration/test_ppo_one_update.py` | `scripts/train_ppo.py`, `docs/THEORY.md` (PPO clipped surrogate section) | `71f0213` | ✅ done |
| §2.3 — **F12** GAE buffer | GAE(λ) advantage buffer with λ=0.95 (FIXED, `config.ppo.gae_lambda`) and γ=0.99 (`config.ppo.gamma`) per Schulman et al. 2016 (arXiv:1506.02438). Advantage normalisation applied before policy update. | `src/services/gae_buffer.py` | `tests/unit/services/test_gae_buffer.py`, `tests/unit/test_gae_math.py` | `docs/THEORY.md` (GAE-λ derivation section) | `71f0213` | ✅ done |
...
| §2.3 — **F14** encoder | State encoder over (A, X, edge_attrs) — GraphSAGE primary (variable-\|V\| via PyG DataLoader) with MLP V_max=512 padding **FALLBACK only** gated by ADR-004 + ADR-008. | `src/model/encoder.py` | `tests/unit/model/test_encoder.py`, `tests/unit/test_encoder_parity.py` | `docs/adr/ADR-004-graphsage-vs-mlp-encoder.md`, `docs/adr/ADR-008-sb3-variable-v-buffer.md` | `71f0213` | ✅ done |
...
| §3 before/after (**F17** writer + **F10** screenshots + **D9** before-evidence) | Obsidian Vault writer (F17 — `src/services/vault_writer.py`, idempotent vault materialisation, brief §2.1 / §3) plus the pre-refactor hero shot landed Phase 1 (**D9**: `results/figures/obsidian_before.png`). "after" shot (closes F10) pending post-refactor in Phase 2. | `src/services/vault_writer.py`, `scripts/capture_obsidian_stub.py` | `tests/architecture/test_graphify_contract.py`, `tests/unit/services/test_vault_writer.py` | `results/vault/`, `results/figures/obsidian_before.png` (✅ Phase 1, **D9**); `results/figures/obsidian_after.png` (⬜ Phase 2+, F10) | `0165fa2` | 🟡 before-shot landed (D9 ✅); after-shot pending (F10 partial) |
