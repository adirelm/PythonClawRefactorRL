...
| Brief § | Requirement | Code file | Test file | Chart / artifact | Commit SHA | Status |
|---|---|---|---|---|---|---|
| §2.2 — **F5** | Custom graph environment exposing `step`/`reset` over the refactored graph state — **NO Gymnasium (brief §2.2 ban)**. State = (A, X, edge_attrs); actions = split_module / merge_modules / rewire_edge / noop; A_max=45057. | `src/env/skills_graph_env.py`, `src/env/state.py`, `src/env/actions.py`, `src/env/action_mask.py` | `tests/unit/test_graph_env_contract.py`, `tests/architecture/test_env_no_gym.py` | `docs/diagrams/env_state_diagram.png`, `scripts/run_episode_smoke.py` | `<phase2-commit>` | ✅ done |
| §2.2 — **F6** | Reward shaping `R_t = α·ΔModularity_t + β·ΔCohesion_t − γ·Coupling_Penalty_t + P_skills_t` with α=1.0, β=1.0, γ=0.5, **P_skills = −5.0** (lazy-load-break penalty, NEGATIVE applied when lazy-load monitor detects break); full ablation matrix ≥5 seeds/cell. | `src/env/reward.py`, `scripts/run_ablation.py` | `tests/unit/test_reward_shape.py`, `tests/architecture/test_reward_formula.py`, `tests/integration/test_ablation_matrix.py` | `results/ablation/reward_ablation_heatmap.png`, `results/ablation/seed_table.csv`, `docs/adr/ADR-007-reward-upgrade-MUST.md`, `docs/prd/PRD-SKILLS.md` | `<phase2-commit>` | ✅ done (Phase 2 reward impl); ablation matrix ⬜ Phase 4 |
| §2.2 — **F7** modularity | Modularity metric service consumed by reward `ΔModularity` term. | `src/services/metrics/modularity.py` | `tests/unit/test_modularity.py` | n/a (service module) | `<phase2-commit>` | ✅ done |
| §2.2 — **F8** cohesion | Cohesion metric service consumed by reward `ΔCohesion` term. | `src/services/metrics/cohesion.py` | `tests/unit/test_cohesion.py` | n/a (service module) | `<phase2-commit>` | ✅ done |
| §2.2 — **F9** coupling | Coupling metric service consumed by reward `Coupling_Penalty` term. | `src/services/metrics/coupling.py` | `tests/unit/test_coupling.py` | n/a (service module) | `<phase2-commit>` | ✅ done |
| §2.2 — **F10** betweenness centrality | Centrality service: Degree per MDP step (cheap); Betweenness **exactly twice per seed** (training start + training end) via `CentralityScheduler`, aggregated across ≥5 seeds with mean ± std + 95% CI for both endpoints AND Δ. | `src/services/centrality.py` | `tests/unit/test_betweenness_seed_stability.py`, `tests/architecture/test_betweenness_call_count.py` | `results/graphs/betweenness_ci.png`, `results/graphs/betweenness_table.csv`, `docs/adr/ADR-006-multi-seed-eval-discipline.md` | `<phase2-commit>` | ✅ done (service + scheduler); CI artifact ⬜ Phase 3 |
...
- ~~`src/env/skills_graph_env.py` + `src/env/state.py` + `src/env/actions.py` + `src/env/action_mask.py` + `src/env/reward.py` — F5/F6, Phase 2 will author.~~ ✅ landed in Phase 2 at `<phase2-commit>`.
- ~~`src/services/metrics/{modularity,cohesion,coupling}.py` — F7/F8/F9, Phase 2 will author.~~ ✅ landed in Phase 2 at `<phase2-commit>`.
- ~~`src/services/centrality.py` (+ `CentralityScheduler`) — F10, Phase 2 will author.~~ ✅ landed in Phase 2 at `<phase2-commit>`.
- ~~`tests/architecture/test_env_no_gym.py` + `tests/architecture/test_betweenness_call_count.py` — Phase 2 architecture tests (active, not xfail).~~ ✅ landed in Phase 2 at `<phase2-commit>`.
- ~~`scripts/run_episode_smoke.py` — Phase 2 episode smoke.~~ ✅ landed in Phase 2 at `<phase2-commit>`.
