...
| §2.2 — **F10** betweenness centrality | Centrality service: Degree per MDP step (cheap); Betweenness **exactly twice per seed** (training start + training end) via `CentralityScheduler`, aggregated across ≥5 seeds {42, 7, 123, 314, 271} with mean ± std + 95% CI for both endpoints AND Δ. | `src/services/centrality.py` | `tests/architecture/test_betweenness_call_count.py`, `tests/architecture/test_seeds_discipline.py` | `docs/adr/ADR-006-multi-seed-eval-discipline.md`, `results/figures/betweenness_ci.png`, `results/data/betweenness_table.csv` | `<phase3-commit>` | ✅ service + scheduler + CI chart + table |
| §2.3 | PPO (Schulman 2017, arXiv:1707.06347) + GAE(λ) (Schulman 2016, arXiv:1506.02438) with proper advantage normalisation; SB3 buffer spike timeboxed 2h, padding-to-V_max=512 fallback gated by ADR-008. | `src/services/ppo_trainer.py`, `src/services/gae_buffer.py`, `src/model/policy_net.py`, `src/model/encoder.py` | `tests/unit/test_gae_math.py`, `tests/integration/test_ppo_one_update.py` | `results/training/ppo_learning_curve.png`, `docs/prd/PRD-PPO.md`, `docs/prd/PRD-GAE.md`, `docs/adr/ADR-008-sb3-variable-v-buffer.md` | `<phase3-commit>` | ✅ trainer + GAE + policy + encoder landed Phase 3 |
| §2.3 — **F11** PPO trainer | PPO clipped-surrogate trainer with ε=0.2 (FIXED), value loss, entropy bonus. Wraps our 4-tuple `SkillsGraphEnv` directly — **does NOT depend on gymnasium** (brief §2.2 ban extends to trainer). Step contract: `state, reward, done, info = env.step(action)`. | `src/services/ppo_trainer.py` | `tests/unit/services/test_ppo_trainer.py`, `tests/integration/test_ppo_one_update.py` | `scripts/train_ppo.py`, `docs/THEORY.md` (PPO clipped surrogate section) | `<phase3-commit>` | ✅ done |
| §2.3 — **F12** GAE buffer | GAE(λ) advantage buffer with λ=0.95 (FIXED, `config.ppo.gae_lambda`) and γ=0.99 (`config.ppo.gamma`) per Schulman et al. 2016 (arXiv:1506.02438). Advantage normalisation applied before policy update. | `src/services/gae_buffer.py` | `tests/unit/services/test_gae_buffer.py`, `tests/unit/test_gae_math.py` | `docs/THEORY.md` (GAE-λ derivation section) | `<phase3-commit>` | ✅ done |
| §2.3 — **F13** policy net | Actor-critic policy network — shared torso, Categorical action head over `A_max=45057`, value head V_φ(s). Logit masking via `−∞` per Huang & Ontañón 2022. | `src/model/policy_net.py` | `tests/unit/model/test_policy_net.py` | `docs/THEORY.md` (actor-critic head + value head section) | `<phase3-commit>` | ✅ done |
| §2.3 — **F14** encoder | State encoder over (A, X, edge_attrs) — GraphSAGE primary (variable-\|V\| via PyG DataLoader) with MLP V_max=512 padding **FALLBACK only** gated by ADR-004 + ADR-008. | `src/model/encoder.py` | `tests/unit/model/test_encoder.py`, `tests/unit/test_encoder_parity.py` | `docs/adr/ADR-004-graphsage-vs-mlp-encoder.md`, `docs/adr/ADR-008-sb3-variable-v-buffer.md` | `<phase3-commit>` | ✅ done |
