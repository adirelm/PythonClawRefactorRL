...
| §2.2 — **F10** betweenness centrality | Centrality service: Degree per MDP step (cheap); Betweenness **exactly twice per seed** (training start + training end) via `CentralityScheduler`, aggregated across ≥5 seeds {42, 7, 123, 314, 271} with mean ± std + 95% CI for both endpoints AND Δ. 5-seed retrain in flight; commit `71f0213` captured seeds 42 + 7 only; seeds 123 / 314 / 271 hit `Categorical(-inf)` corner case in `policy_net.get_action` (degenerate all-False action_mask post-rollout) — fix in flight (Phase 3 rework, R1 NOOP-slot pin + R2 5-seed retrain). See **Known gaps** in `docs/TODO.md`. | `src/services/centrality.py` | `tests/architecture/test_betweenness_call_count.py`, `tests/architecture/test_seeds_discipline.py` | `docs/adr/ADR-006-multi-seed-eval-discipline.md`, `results/figures/betweenness_ci.png`, `results/data/betweenness_table.csv` | `71f0213` | 🟡 in-progress (2 of 5 seeds; 5-seed retrain in flight) |
