...
| §2.2 — **F10** betweenness centrality | Centrality service: Degree per MDP step (cheap); Betweenness **exactly twice per seed** (training start + training end) via `CentralityScheduler`, aggregated across ≥5 seeds with mean ± std + 95% CI for both endpoints AND Δ. | `src/services/centrality.py` | `tests/architecture/test_betweenness_call_count.py`, `tests/architecture/test_seeds_discipline.py` | `docs/adr/ADR-006-multi-seed-eval-discipline.md` (CI chart `results/graphs/betweenness_ci.png` + table `results/graphs/betweenness_table.csv` ⬜ Phase 3+) | `ec1288a` | 🟡 service + scheduler ✅; CI chart + table pending Phase 3+ |
